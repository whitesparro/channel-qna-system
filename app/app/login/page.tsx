...
export default function LoginPage() {
  ...
  const login = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    if (!displayName.trim() || !password) { setError("All fields are required."); return; }
    setLoading(true);
  ...
  return (
    <div style={S.page}>
      <div style={S.box}>
        <h2 style={{ margin: "0 0 6px", color: "#fff", textAlign: "center" }}>Welcome back</h2>
        <p style={{ color: "#888", textAlign: "center", marginBottom: 28, marginTop: 0 }}>Sign in to Channel Q&A</p>
        ...
        <form onSubmit={login}>
          ...
          <input id="displayName" value={displayName} onChange={(e) => setDisplayName(e.target.value)}
            placeholder="Your username" autoComplete="username" style={S.input} />

          <label style={S.label} htmlFor="password">Password</label>
          <input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)}
            placeholder="Your password" autoComplete="current-password" style={S.input} />
        ...
        <p style={{ textAlign: "center", color: "#666", marginTop: 20, fontSize: 14 }}>
          No account? <Link href="/signup" style={{ color: "#7c6af7" }}>Sign up</Link>
        </p>
